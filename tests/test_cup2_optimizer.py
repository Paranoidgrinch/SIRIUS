...
def test_cup2_primary_rcds_production_policy_has_derived_measurement_budget():
    ...
    # Live Cup-2 optimization must re-measure repeated points.
    assert (
    ...
    # In the current RCDS implementation a line grid contains
    # line_samples values and may additionally insert alpha=0.
    #
    # With cache reuse disabled that origin is a REAL repeated
    # machine measurement rather than a cache hit.
    max_grid_evaluations_per_line = (
        policy.line_samples
        + 1
    )

    # One additional real evaluation may be generated by the
    ...
    # direction are searched per RCDS iteration.
    ...
    # fresh Cup-2 measurement after optimizer.optimize().
    assert (
        policy.max_evaluations
        + 1
